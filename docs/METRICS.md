...
| `idemio_request_duration_seconds` | histogram | `route` | Subtracting the downstream duration from this **is** the budgeted overhead, so a regression can be attributed rather than only noticed. Labelled by route pattern, never by path, to keep caller-controlled cardinality out of a histogram. | [SYSTEM_DESIGN.md](SYSTEM_DESIGN.md) |
