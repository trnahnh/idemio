...
|---|---|---|
| [SYSTEM_DESIGN.md](SYSTEM_DESIGN.md) | Architecture, write flow, failure modes, HA, security, and why each signal exists | Onboarding, or changing how a request is processed |
...
| [METRICS.md](METRICS.md) | Metric names, labels, and the alert each one carries | Wiring an alert, or adding a metric |
...
`API_REFERENCE.md`. Metric names live only in `METRICS.md`. Other documents link rather than restate. If you find the same fact
